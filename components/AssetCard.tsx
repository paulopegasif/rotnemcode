...
import { Layout, Layers, FileCode, Code2, Terminal, FileJson, Eye, MoreVertical, Copy } from 'lucide-react';
...
export const AssetCard: React.FC<{ item: AssetItem; isFavorite?: boolean; onToggleFavorite?: (id: string) => void; highlight?: string; }> = ({ item, isFavorite = false, onToggleFavorite, highlight }) => {
  ...
  const renderTitle = () => {
    ...
    return <>{before}<mark className="bg-yellow-300/60 dark:bg-yellow-400/30 rounded px-0.5" aria-label={`Termo buscado: ${match}`}>{match}</mark>{after}</>;
  };
    ...
    <Card className="group overflow-hidden flex flex-col hover:shadow-md transition-all duration-200">
      <div className="h-40 bg-muted w-full flex items-center justify-center relative overflow-hidden">
        ...
        <div className="absolute top-2 right-2 flex gap-2">
          {item.status && (
            <Badge variant={item.status === 'Pro' ? 'default' : 'secondary'} className="bg-background/80 backdrop-blur-sm shadow-sm">
              {item.status}
            </Badge>
          )}
      ...
      <div className="p-4 flex flex-col flex-grow">
        <h3 className="font-semibold text-lg leading-tight mb-1 truncate" title={item.title}>{renderTitle()}</h3>
        ...
        {showPreview && item.code && (
          <pre className="mt-4 p-3 rounded-md bg-slate-900 text-slate-100 text-xs overflow-auto max-h-48" aria-label="Pré-visualização do código"><code>{item.code}</code></pre>
        )}
      </div>
    </Card>
  );
